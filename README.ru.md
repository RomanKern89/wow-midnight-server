<!-- Язык: [English](README.md) · **Русский** -->

![WoW Midnight Server](assets/banner.png)

# WoW Midnight сервер — TrinityCore 12.0.7 (билд 68974)

> Документированный приватный сервер **World of Warcraft: Midnight** на базе
> **TrinityCore master**, на ретейл-линейке **12.0.7**, плюс набор оригинальных
> серверных фиксов, которые делают новейший контент реально играбельным.

> **Про номера билдов:** всё здесь написано под билд **12.0.7.68974**, который
> был живым ретейл-билдом на 2026-07-05. Retail двигается раз в несколько недель,
> а поставить старый билд не может ничто — значит клиент, скачанный сегодня,
> будет новее. Настраивайте сервер на тот билд, который реально можете получить,
> см. **[CLIENT.md](docs/ru/CLIENT.md)**.

**🌐 Язык:** [English](README.md) · **Русский**

> ⚠️ **Сначала прочти:** репозиторий содержит **только документацию и наши
> оригинальные фиксы**. В нём **нет** игрового клиента, игровых данных и базы
> мира — это собственность Blizzard, распространять её нельзя. Всё это ты
> собираешь сам из своего ретейл-клиента. См. **[DISCLAIMER.md](DISCLAIMER.md)**.

---

## Чем проект отличается от других

Большинство публичных приватных серверов WoW работают на **старых аддонах**
(Wrath 3.3.5a, Cata, MoP…). Этот нацелен на **актуальный ретейл — патч 12.0.7
«Midnight»** и держится честным и играбельным:

| | Этот проект | Типичный приватный сервер |
|---|---|---|
| **Билд клиента** | **12.0.7 (Midnight)** — актуальная ретейл-линейка | 3.3.5a / 4.3.4 / 5.4.8 |
| **Расы** | Все 26, **союзные расы разблокированы**, работает **интро Драктиров-Эвокеров** | Только классические |
| **Охват контента** | Vanilla → Midnight в одной БД, заполнено **517 карт** | Один аддон |
| **Живой мир** | **60 самостоятельных ботов**: дерутся, добывают, плавят, куют, торгуют, проходят квесты и разговаривают — без единого игрока в сети | Обычно нет |
| **Политика данных** | Максимизировано до **потолка публичных данных**, пробелы честно задокументированы | Часто молча сломано |
| **Фиксы** | Курируемые, обратимые, на безопасных ID-бандах **community-фиксы** (этот репо) | Разрозненные |

### Ключевое
- **Мир, населённый без игроков** — см. [`bots/`](bots/README.md).
  Боты без сетевого подключения, с собственной логикой решений: охотятся, копают
  руду и рвут травы, плавят и куют из добытого, сбывают хлам и чинятся по слотам,
  выставляют товар на аукцион и надевают купленное, берут квесты и идут к их целям,
  сбиваются в группы, разговаривают, седлают маунтов на дальних перегонах, уходят
  из зоны, когда её переросли, и переживают перезапуск сервера, не теряя прогресса.
  Каждое утверждение в том README — измерение, включая неутешительные.
- **Новейший контент, до которого реально дойти** — интро союзных рас и классов,
  современные зоны (Khaz Algar, Isle of Dorn, Harandar) с воскрешением и квестами.
- **Курируемый пак фиксов** — 12 файлов: починка цепочек квестов, спавн объектов,
  блокирующих квесты, привязки лок-аутов/журнала легаси-рейдов, кладбища
  новейших зон, праздники, привязанные к реальному календарю, убранные из
  стартовых зон рейдовые боссы, убранные задвоенные спавны и починка NPC выше.
  Всё аддитивное, обратимое, на пустых ID-бандах — не конфликтует со стоковой
  установкой. Каждый фикс проверен так: реальная база ломалась, чинилась, фикс
  применялся повторно, затем откатывался — и исходные числа возвращались
  в точности.
- **Честная инженерия** — где движок упирается в реальный потолок (боевой ИИ
  боссов, скриптинг новейших фазовых сцен), это задокументировано, а не подделано.

---

## Контент в цифрах (проверенные значения БД)

| Метрика | Кол-во |
|---|---:|
| Квесты | **48 257** |
| Цели квестов | 61 101 |
| Дающие / принимающие квесты (NPC) | 27 695 / 34 525 |
| Шаблоны существ / спавны | 227 684 / **730 180** |
| Шаблоны объектов / спавны | 89 967 / 197 732 |
| Карты со спавнами | **517** |
| Строк таблиц лута | 3 084 867 |
| Строк вендоров | 172 414 |
| Ретейл-билд | **12.0.7.68974** |

---

![Статистика контента](assets/stats.png)

## Галерея

### Архитектура

![Архитектура](assets/architecture.png)

### Игровые скриншоты

> Это заглушки — положи свои реальные игровые скриншоты в
> `assets/screenshots/`, и они появятся здесь. (Мы **не** включаем игровые
> изображения Blizzard; см. [DISCLAIMER](DISCLAIMER.md).)

<!--
![Создание персонажа](assets/screenshots/01-character.png)
![Isle of Dorn](assets/screenshots/02-isle-of-dorn.png)
![Спутники-боты](assets/screenshots/03-bots.png)
-->

_Скриншотов пока нет — вклад приветствуется._

## Быстрый старт

0. **Получи клиент** — скачиваешь у Blizzard сам, бесплатно. Учти: ты всегда
   получаешь тот билд, который **живой прямо сейчас**; поставить старый не может
   ничто, поэтому проверь до загрузки 100 ГБ.
   → **[docs/ru/CLIENT.md](docs/ru/CLIENT.md)**
1. **Собери ядро** — TrinityCore master. → [docs/ru/SETUP.md](docs/ru/SETUP.md)
2. **Извлеки игровые данные** из своего ретейл-клиента (maps/vmaps/mmaps/dbc).
3. **Импортируй базу мира**, запусти `worldserver` + `bnetserver`.
4. **Применить community-фиксы** из [`sql/`](sql/). → [sql/README.md](sql/README.md)
5. **Подключи** клиент. → [docs/ru/CONNECT.md](docs/ru/CONNECT.md)

### Короткий путь (два скрипта)

```powershell
# на ТВОЁМ ПК — ставит retail-клиент и направляет его на сервер.
# Сверяет билд с серверным ДО загрузки и останавливается, если они разные.
.\scripts\setup-client.ps1 -InstallDir C:\Games\WoW -ServerIP 192.168.1.50
```
```bash
# на СЕРВЕРЕ — создаёт логин, которым реально можно войти, с правами GM
SOAP_USER='1#1' SOAP_PASS='...' scripts/create-account.sh -e you@example.com -p secret -g 3
```
Имя аккаунта — **почта**: retail-клиент входит через Battle.net, поэтому
`account create` создаёт то, чем войти нельзя. См.
[CONNECT.md](docs/ru/CONNECT.md).

> **Где взять клиент?** У Blizzard, бесплатно, самостоятельно — см.
> [CLIENT.md](docs/ru/CLIENT.md). Пожалуйста, не просите и не выкладывайте
> ссылки на скачивание клиента: это нарушение авторских прав, сторонние сборки —
> непроверяемые бинарники, а официальная загрузка делается одной командой.

Полный гайд: **[docs/ru/SETUP.md](docs/ru/SETUP.md)** ·
Возможности и контент: **[docs/ru/FEATURES.md](docs/ru/FEATURES.md)** ·
Детали фиксов: **[docs/ru/FIXES.md](docs/ru/FIXES.md)**

---

## Структура репозитория

```
.
├── README.md / README.ru.md      Обзор проекта (EN / RU)
├── CHANGELOG.md                  Что нового — по замерам
├── DISCLAIMER.md                 Юридическое — что можно и что нельзя публиковать
├── LICENSE                       MIT (только оригинальные материалы)
├── docs/
│   ├── en/  CLIENT · SETUP · CONNECT · FEATURES · FIXES · ANNOUNCEMENT
│   └── ru/  CLIENT · SETUP · CONNECT · FEATURES · FIXES · ANNOUNCEMENT
├── bots/                         Система ботов (72 скрипта + патч ядра)
│   ├── README.md                 Что они умеют — по замерам, включая неудачи
│   ├── COMMANDS.md / .ru.md      Все команды и настройки с пояснениями
│   ├── src/                      Кладётся в src/server/scripts/Custom/Bots/
│   ├── custom_script_loader.cpp  Заменяет пустую заглушку TC, иначе не подключатся
│   └── core-patch/               Единственный патч ядра, девять строк
├── docker/                       Развёртывание в compose (руководство — INSTALL.md)
├── sql/                          Community-фиксы (обратимые, на безопасных ID-бандах)
└── scripts/
    ├── setup-client.ps1          Установка retail-клиента, проверка билда, настройка на сервер
    ├── create-account.sh         Создание Battle.net-аккаунта (+ уровень GM) на живом сервере
    ├── apply_fixes.sh            Применение пака SQL-фиксов
    └── vmssh.py                  SSH-хелпер (креды из переменных окружения)
```

---

## Вклад

Issues и PR приветствуются — особенно дополнительные **обратимые, на безопасных
ID-бандах** фиксы данных для новейшего контента. Никогда не коммить секреты и
игровые ассеты Blizzard (`.gitignore` защищает от обоих). См. [DISCLAIMER.md](DISCLAIMER.md).

## Благодарности

Собрано на [TrinityCore](https://www.trinitycore.org/) (GPL-2.0). World of
Warcraft® © Blizzard Entertainment. Фанатский, некоммерческий, образовательный
проект.
